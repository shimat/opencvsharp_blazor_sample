...
| `/samples/aruco` | Generates an ArUco marker and composites it onto a photo at a random position/rotation; detection is a known-issue preview (see below) |
...
- Any algorithm that internally allocates a `cv::UMat` (e.g. `ORB`, `CascadeClassifier.DetectMultiScale`, `Aruco.ArucoDetector.DetectMarkers`, `Cv2.Stitcher.Stitch`) hangs or hard-aborts, because `cv::ocl::haveOpenCL()` throws instead of returning `false` on this wasm build ([opencvsharp#2037](https://github.com/shimat/opencvsharp/issues/2037), fix in progress in [opencvsharp#2041](https://github.com/shimat/opencvsharp/pull/2041)) — the root cause is a missing `-DWITH_OPENCL=OFF` in OpenCvSharp's wasm build CMake configuration, so it can't be worked around from this app until that fix lands in a released package. The `/samples/features` page avoids this by using AKAZE instead of ORB; the `/samples/detection`, `/samples/aruco`, and `/samples/stitching` pages (which need `CascadeClassifier`, `ArucoDetector`, and `Cv2.Stitcher` respectively) are kept as previews with an on-page notice
